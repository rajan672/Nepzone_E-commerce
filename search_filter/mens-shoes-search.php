...
$record = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM men_shoes"));
$pagi = ceil($record / $per_page);

$sql = "SELECT * FROM men_shoes ORDER BY id DESC limit $start,$per_page";
                  ...
                  while ($row = mysqli_fetch_assoc($res)) {
                    ...
                    echo '
                        <div class="col-md-3 col-sm-4 col-6">
                          <a href="../product-details/men-shoes-product.php?id=' . $mens_id . '" class="product">
                            <div class="product-top">
                              <i class="fa fa-heart float-right"></i>
                              <img class="img-fluid" src=".' . str_replace('..', '.', $row['mens_image']) . '" alt="">
                            </div>

                            <div class="product-bottom">
                              <div class="product-desc-top">
                                <div class="product-brand text-secondary">' . $row['mens_brand'] . '</div>
                                <div class="product-desc text-dark">' . $row['mens_name'] . '</div>
                                <div class="product-price">
                                  <span><strong> NRP. ' . $row['mens_selling_price'] . ' </strong></span> <br>
                                  <span class="text-secondary"><strike> NRP.' . $row['mens_market_price'] . '  </strike></span> <br>
                                  <span class="text-success"><small> <strong> ' . $row['mens_discount'] . '% Off</strong> </small></span>
                                </div>
                              </div>
                              <div class="product-desc-bottom">
                                <div class="product-desc-extra">
                                  <span class="badge badge-success"> ' . $row['mens_rating'] . ' <i class="fas fa-star"></i></span> <span class="text-secondary"> (' . $row['mens_review'] . ') </span> <br>
