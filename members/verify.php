...
if (isset($_POST['vkey'])) {
    ...
    $vkey = $_POST['vkey'];
    $resultset = $conn->query("SELECT verifed,vkey FROM register WHERE verifed = 0 AND vkey = '$vkey' LIMIT 1");
    if ($resultset->num_rows == 1) {
        //validate the email
        $update = $conn->query("UPDATE register SET verifed = 1 WHERE vkey = '$vkey' LIMIT 1");
        if ($update) {
            echo " <p class = 'text-sucess'>Your account has been verified. you may noe Login. </p>";
        } else {
            echo $conn->error;
        }
    } else {
    ...
    die('<div class="alert alert-success col-sm-6 mi-5 mt2">Something went wrong..</div>');
...
<body>
    ...
    <div class="text-center p-4 m-4">
        <h4>Go for <a href="login.php"> Login</a></h4>
    </div>
